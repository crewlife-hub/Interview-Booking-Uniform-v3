...
/**
 * Public API: return job list for a brand with CL code details suitable for the client.
 * @param {string} brand
 * @returns {Array} Array of { jobTitle, defaultCLCode, bookingUrl, active }
 */
function getJobsForBrandApi(brand) {
  var jobs = getJobsForBrand_(brand || '');
  var out = [];
  for (var i = 0; i < jobs.length; i++) {
    var j = jobs[i];
    var clDet = getCLCodeDetails_(brand, j.defaultCLCode) || {};
    out.push({
      jobTitle: j.jobTitle || '',
      defaultCLCode: j.defaultCLCode || '',
...
/**
 * Public API: return all CL codes for a brand (for dropdown).
 * Falls back to JOBS if CL_CODES is empty.
 * @param {string} brand
 * @returns {Array} Array of { clCode, recruiterName, bookingUrl, active, jobTitle }
 */
function getCLCodesForBrandApi(brand) {
  var codes = getCLCodesForBrand_(brand || '');
  var jobs = getJobsForBrand_(brand || '');
  var out = [];
  var seen = {};
  // Add CL codes from CL_CODES tab
  for (var i = 0; i < codes.length; i++) {
    var c = codes[i];
    if (!c.clCode) continue;
    var key = String(c.clCode).toUpperCase();
    if (seen[key]) continue;
    seen[key] = true;
    // Find matching job title if any
    var jt = '';
    for (var j = 0; j < jobs.length; j++) {
      if (String(jobs[j].defaultCLCode).toUpperCase() === key) {
        jt = jobs[j].jobTitle || '';
        break;
      }
    }
  ...
  // If no CL codes, fallback to JOBS tab
  if (out.length === 0) {
    for (var k = 0; k < jobs.length; k++) {
      var jb = jobs[k];
      if (!jb.defaultCLCode) continue;
      var kk = String(jb.defaultCLCode).toUpperCase();
      if (seen[kk]) continue;
      seen[kk] = true;
      var clDet = getCLCodeDetails_(brand, jb.defaultCLCode) || {};
      out.push({
        clCode: jb.defaultCLCode,
        recruiterName: clDet.recruiterName || '',
        bookingUrl: clDet.bookingUrl || '',
        active: !!jb.active,
