...
function validateOtp_(params) {
  ...
  if (rowOtp === otp) {
    ...
    // Resolve booking URL: prefer Position Link stored on token row, then CL_CODES lookup
    var clResolution = resolveCLCodeFromTextForEmail_(rowBrand, rowTextForEmail);
    var posIdx = idx['Position Link'];
    var storedPos = (posIdx !== undefined) ? String(targetRowData[posIdx] || '').trim() : '';
    
    // If Position Link stored on token row, use it (overrides CL_CODES)
    if (storedPos && storedPos.indexOf('http') === 0) {
      clResolution = clResolution || {};
      clResolution.ok = true;
      clResolution.bookingUrl = storedPos;
      clResolution.source = 'POSITION_LINK';
      Logger.log('[validateOtp_] Using stored Position Link: %s', storedPos.substring(0, 60));
    } else if (clResolution && clResolution.ok && clResolution.bookingUrl) {
      clResolution.source = 'CL_CODES';
      Logger.log('[validateOtp_] Using CL_CODES booking URL: %s', clResolution.bookingUrl.substring(0, 60));
    } else {
      Logger.log('[validateOtp_] WARNING: No booking URL resolved. clResolution=%s', JSON.stringify(clResolution));
    }
