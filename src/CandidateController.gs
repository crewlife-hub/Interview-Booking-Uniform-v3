...
/**
 * Handle booking confirmation POST (token consume + redirect)
 * @param {Object} params - POST parameters
 * @param {string} traceId - Trace ID
 * @returns {HtmlOutput}
 */
function handleBookingConfirmPost_(params, traceId) {
  ...
  if (!validation.ok) {
    logEvent_(traceId, '', '', 'BOOKING_TOKEN_INVALID', { error: validation.error, code: validation.code });
    return serveErrorPage_('Invalid Link', validation.error, traceId);
  }

  var tokenRow = getTokenRow_(token);
  if (!tokenRow.ok) {
    return serveErrorPage_('Invalid Link', 'Token not found.', traceId);
  }

  var map = tokenRow.map;
  var row = tokenRow.row;
  var brand = String(row[map['Brand']] || '').toUpperCase();
  var email = String(row[map['Email']] || '').toLowerCase();
  var textForEmail = String(row[map['Text For Email']] || '').trim();

  var bookingUrl = resolveBookingUrlForCandidate_(brand, email, textForEmail, traceId);
  if (!bookingUrl) {
    logEvent_(traceId, brand, email, 'BOOKING_URL_MISSING', { result: 'ERROR', message: 'No booking URL' });
    return serveErrorPage_('Configuration Error', 'No booking URL configured for this position. Please contact your recruiter.', traceId);
  }

  // Mark token as USED
  var sheet = getConfigSheet_().getSheetByName('TOKENS');
  sheet.getRange(tokenRow.rowIndex, map['Token Status'] + 1).setValue('USED');
  if (map['Used At'] !== undefined) {
    sheet.getRange(tokenRow.rowIndex, map['Used At'] + 1).setValue(new Date());
  }

  logEvent_(traceId, brand, email, 'TOKEN_USED', {
    tokenLast6: token.slice(-6),
    result: 'OK',
    message: 'Token consumed'
  });

  return serveRedirectPage_(bookingUrl, brand);
}

/**
 * Resolve booking URL using Smartsheet row + config fallbacks
 * @param {string} brand - Brand code
 * @param {string} email - Candidate email
 * @param {string} textForEmail - Text For Email
 * @param {string} traceId - Trace ID
 * @returns {string} Booking URL or empty
 */
function resolveBookingUrlForCandidate_(brand, email, textForEmail, traceId) {
  var clCode = '';

  // 1) Try to read CL Code from Smartsheet row
  var candidate = searchCandidateInSmartsheet_(brand, email, textForEmail);
  if (candidate.ok && candidate.found && candidate.exactMatch && candidate.candidate) {
    clCode = extractClCodeFromRow_(candidate.candidate);
  }

  // 2) Use CL code from row if possible
  if (clCode) {
    var clDetails = getCLCodeDetails_(brand, clCode);
    if (clDetails && clDetails.active && clDetails.bookingUrl) return clDetails.bookingUrl;
  }

  // 3) Resolve via JOBS/CL_CODES mapping
  var resolution = resolveCLCodeFromTextForEmail_(brand, textForEmail);
  if (resolution && resolution.ok && resolution.bookingUrl) return resolution.bookingUrl;

  // 4) Fallback to BRAND_CONFIG default booking URL
  var cfg = getBrandConfig_(brand);
  if (cfg && cfg.defaultBookingUrl) return cfg.defaultBookingUrl;

  logEvent_(traceId, brand, email, 'BOOKING_URL_RESOLUTION_FAILED', { result: 'ERROR', message: 'No matching CL code or default URL' });
  return '';
}

/**
 * Extract CL Code from Smartsheet row map
 * @param {Object} row - Row map
 * @returns {string} CL Code
 */
function extractClCodeFromRow_(row) {
  if (!row) return '';
  var keys = Object.keys(row);
  for (var i = 0; i < keys.length; i++) {
    var k = String(keys[i]).toLowerCase();
    if (k.indexOf('cl') !== -1 && k.indexOf('code') !== -1) {
      var val = String(row[keys[i]] || '').trim();
