...
function doGet(e) {
  ...
  try {
    ...
    // Route: Diagnostics
    if (page === 'diag') {
      return serveDiagPage_(brand, params, traceId);
    }
    ...
    // Route: Booking confirmation page (token-gated)
    if (page === 'booking_confirm') {
      return serveBookingConfirmPage_(params, traceId);
    }

    // Route: Legacy token link (fallback)
    if (token) {
      return serveBookingConfirmPage_({ token: token, brand: brand }, traceId);
    }
...
function doPost(e) {
  ...
  try {
    ...
    // Route: Booking confirmation submit (token consume)
    if (action === 'confirmbooking') {
      return handleBookingConfirmPost_(params, traceId);
    }

    // Route: Diagnostics actions
    if (action === 'rundispatcher') {
      return handleRunDispatcher_(params, traceId);
    }
